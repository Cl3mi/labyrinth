...
public class NextPlayerEventListener {
    private static final Logger log = LoggerFactory.getLogger(NextPlayerEventListener.class);
    ...
    public void onEvent(NextPlayerEventListener ignoredEvent) {
        try {
            ...
            log.info("GameStateUpdate event send after NextPlayerEventListener");
        } catch (Exception ex) {
            log.error("Error while handling NextPlayerEventListener", ex);
        }
    }
}